...
// Applies all JSON fields to a UObject, skipping "_ClassName".
// ===========================================================================
...
// ===========================================================================
// SetStructFieldsFromJson
// Like SetPropertiesFromJson but works on raw struct memory (UScriptStruct + void*).
// Handles nested instanced-object arrays and FInstancedStruct arrays that
// FJsonObjectConverter cannot deserialise. Falls back to FJsonObjectConverter
    ...
    const TSharedPtr<FJsonObject>& Json, UObject* Outer, FString& OutErrors)
{
    ...
    for (const auto& Pair : Json->Values)
    {
        ...
        // -------------------------------------------------------------------
        // FArrayProperty — check for instanced object arrays and
        // FInstancedStruct arrays that need manual handling
        // -------------------------------------------------------------------
...
// ===========================================================================
// SetProperty
// Comprehensive property setter covering all UE5 FProperty types.
// ===========================================================================
bool FEpicUnrealMCPPropertyUtils::SetProperty(UObject* Object, const FString& PropertyName,
                                               const TSharedPtr<FJsonValue>& Value, FString& OutError)
{
    ...
    // -----------------------------------------------------------------------
    // 1b. Other FObjectPropertyBase subclasses — always path-string based
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // 8. FStructProperty — use SetStructFieldsFromJson so nested instanced
    //    arrays (e.g. FMassEntityConfig::Traits, MassAssortedFragmentsTrait::
    ...
    // -----------------------------------------------------------------------
    // 9. FArrayProperty — instanced object arrays and FInstancedStruct arrays
    //    get special handling; everything else falls through to FJsonObjectConverter.
    // -----------------------------------------------------------------------
    if (FArrayProperty* ArrProp = CastField<FArrayProperty>(Prop))
    {
        ...
        // Non-instanced array — FJsonObjectConverter handles it
